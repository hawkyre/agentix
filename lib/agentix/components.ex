if Code.ensure_loaded?(Phoenix.Component) do
  defmodule Agentix.Components do
    ...
    Agentix.Components` and render the assigns, or run `mix agentix.gen.components` to
    ...
    dividers. Two small extras the host opts into:
      ...
      * **grouping** — give the thread the `agentix-thread` class to collapse each turn
        ...
        a single header (the CSS lives in `Agentix.Components.css/0`);
    ...
    *and* tool messages) drives turn grouping: every assistant + tool row of one turn
    ...
    # `config :agentix, render_markdown: false`. The actual markdown engine is the host's
    # (wired into the JS hook via `configureMarkdown/1`) — see `agentix_stream_hook.js`.
    defp markdown?, do: Application.get_env(:agentix, :render_markdown, true)

    # The finalized-message text node gets the `AgentixMarkdown` hook only when markdown is
    # enabled and the row has a DOM id (phx-hook requires one). The node carries the raw text
    # in `data-md` (the hook re-renders it) plus a plain-text body for SSR / no-JS.
