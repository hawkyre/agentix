defmodule AgentixComponents do
  ...
  Rendered against the `Agentix.Chat` projection assigns, in a flat Tailwind style
  (stock `neutral` scale + `emerald`/`red`/`amber` semantics, `darkMode: 'class'`). The
  pending controls switch on `pending[id].kind` (`:approval` / `:elicitation` /
  `:client_exec`), not the executor. They emit `"approve"`/`"deny"` (with
  `phx-value-id`) and a `"resolve"` form submit (carrying `tool_call_id` + an
  `answer`/`result` field) — wire those to `Agentix.Chat.resolve/3`.
  """
